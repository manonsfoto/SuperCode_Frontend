...
function main(subEv: Event): void {
  ...
  const g = parseFloat(
    document.getElementById(a)
      ? (document.getElementById(a) as any).value
      : 3000
  );
  const h = parseFloat(
    document.getElementById(b) ? (document.getElementById(b) as any).value : 1
  );
  ...
  if (end === res) {
    ...
    (
      document.getElementById("result") as any
    ).innerText = `Nettogehalt: ${res} €`;
  }
}
